...
from awesome_date_dimension.config import Column, DimFiscalMonthConfig


class TestDimFiscalMonthConfig(unittest.TestCase):
    ...
    def test_creating_duplicate_column_names_with_column_factory_causes_error(self):
        ...
        with self.assertRaises(AssertionError):
            DimFiscalMonthConfig(column_factory=column_factory)
    ...
    def test_prefixing_with_column_factory(self):
        ...
        cfg = DimFiscalMonthConfig(column_factory=column_factory)
    ...
    def test_defaults_are_immutable(self):
        with open("./tests/files/default_dim_fiscal_month_config.pickle", "rb") as file:
            saved_defaults = pickle.load(file)

        self.assertEqual(DimFiscalMonthConfig(), saved_defaults)
