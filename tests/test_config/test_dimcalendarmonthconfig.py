...
from awesome_date_dimension.config import Column, DimCalendarMonthConfig


class TestDimCalendarMonthConfig(unittest.TestCase):
    ...
    def test_creating_duplicate_column_names_with_column_factory_causes_error(self):
        ...
        with self.assertRaises(AssertionError):
            DimCalendarMonthConfig(column_factory=column_factory)
    ...
    def test_prefixing_with_column_factory(self):
        ...
        cfg = DimCalendarMonthConfig(column_factory=column_factory)

        col_field_names = (f.name for f in fields(cfg.columns))
        for f_name in col_field_names:
            col: Column = cfg.columns.__dict__[f_name]
            with self.subTest(
                f"column name should be prefixed with 'CompanyName' if it previously contained 'Calendar': {col.name}"
            ):
                if "Calendar" in f_name:
                    self.assertEqual(name_map[f_name], col.name)

    def test_defaults_are_immutable(self):
        with open(
            "./tests/files/default_dim_calendar_month_config.pickle", "rb"
        ) as file:
            saved_defaults = pickle.load(file)

        self.assertEqual(DimCalendarMonthConfig(), saved_defaults)
